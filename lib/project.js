...
let Project = module.exports = function(opts) {
  ...
  this.integration = Integration.fromOpenAPI(this.openapi);
}
...
Project.prototype.aggregateActions = function() {
  let addAction = (action) => {
    if (typeof action === 'string') {
      action = require(nodepath.join(this.directory, action));
    }
...
function getOpenAPIOperation(method, path, pathOp) {
  let op = {
  ...
  if (!op.parameters) {
    let pathParams = path.match(PATH_PARAM_REGEX) || [];
    op.parameters = pathParams.map(p => ({
      in: 'path',
      required: true,
  ...
  let hasBody = method === 'post' || method === 'patch' || method === 'put';
  if (hasBody) {
    op.parameters.push({
      in: 'body',
      name: 'body',
  ...
  Object.keys(pathOp.action.inputSchema.properties || {}).forEach(prop => {
    ...
    if (param) {
      Object.assign(param, pathOp.action.inputSchema.properties[prop]);
...
Project.prototype.initializeOpenAPI = function(openapi) {
  ...
  for (let path in this.paths) {
    for (let method in this.paths[path]) {
      ...
      this.openapi.paths[path][method] = getOpenAPIOperation(method, path, pathOp);
    }
  }
  return this.openapi;
}
...
Project.prototype.setup = function(app) {
  for (let path in this.paths) {
    for (let method in this.paths[path]) {
      ...
      let op = this.paths[path][method];
      let expressPath = path.replace(PATH_PARAM_REGEX, ':$1');
