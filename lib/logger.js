...
class Logger {
  ...
  static chalkOperation(op) {
    let str = chalk.magenta(op.name);
    let match = op.name.match(/^([A-Z]+) (\/.*)$/);
    if (op.method && op.path) {
      str += '\n' + Logger.chalkMethod(op.method) + '\t' + op.path;
    }
    let desc = op.info.summary || op.info.description;
    if (desc) str += '\n' + chalk.gray(desc);
    return str;
  }
  ...
  static logOperation(op) {
    Logger.log(Logger.chalkOperation(op));
  }
