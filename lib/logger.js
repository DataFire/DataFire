...
class Logger {
  ...
  static chalkOperation(name, op, skipDescription) {
  ...
  static logSchema(schema, indent, name) {
    ...
    let toLog = name ? indent + Logger.padString(name + ': ', 14) : '';
    toLog += Logger.chalkType(schema.properties ? 'object' : schema.type);
    if (schema.items) {
      toLog += '[' + Logger.chalkType(schema.items.type) + ']';
    }
    if (schema.description) {
      let desc = chalk.gray(Logger.truncate(schema.description, 60));
      toLog += '\n' + indent + desc + '\n';
    }
  ...
  static logDescription(str) {
    ...
    str = truncate(str.trim(), MAX_DESCRIPTION_LENGTH);
