...
const INTEGRATION_LOCATIONS = require('./locations').integrations;
...
class Integration {
  ...
  static new(name) {
    ...
    let tryOpen = (baseDir) => {
      let filename = path.join(baseDir, name, 'integration');
      if (!fs.existsSync(filename + '.js') && !fs.existsSync(filename + '.json')) return;
      let spec = require(filename);
