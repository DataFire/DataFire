...
const NATIVE_DIR = path.join(__dirname, '..', 'native_integrations');
...
class Integration {
  ...
  static new(name) {
    ...
    let tryOpen = (dir) => {
      let filename = path.join(dir, name + '.openapi.json');
      let spec = null;
      try {
    ...
    let integration = tryOpen(datafire.integrationsDirectory) || tryOpen(NATIVE_DIR);
    if (!integration) throw new Error("Integration " + name + " not found. Please run:\ndatafire install " + name);
    return integration;
  }
