...
let maybeGetResponseError = (body) => {
  let err = body.errorMessage || body.error_message || body.error;
...
let getCollectionFormatSeparator = (cf) => {
  if (cf === 'csv') return ',';
  if (cf === 'ssv') return ' ';
  if (cf === 'tsv') return '\t';
  if (cf === 'pipes') return '|';
...
SwaggerClient.prototype.refreshOAuth = function(cb) {
  var self = this;
  let definition = self.swagger.securityDefinitions[self.auth.securityDefinition];
  ...
  Request.post(definition.tokenUrl, {form: form, json: true}, (err, resp, body) => {
...
SwaggerClient.prototype.request = function(method, path, answers, callback) {
  ...
  var addParameter = function(param, ans) {
    if (param.collectionFormat && Array.isArray(ans)) {
      // FIXME: support multiple parameters with different collection formats
      if (param.collectionFormat === 'multi') {
        req.qsStringifyOptions.arrayFormat = 'repeat';
      } else {
        req.qsStringifyOptions.sep = getCollectionFormatSeparator(param.collectionFormat);
      }
    }
    if (param.in === 'header') {
