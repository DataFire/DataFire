...
var maybeGetResponseError = function maybeGetResponseError(body) {
  var err = body.errorMessage || body.error_message || body.error;
...
SwaggerClient.prototype.initialize = function (callback) {
  ...
  SwaggerParser.parse(self.swagger).then(function (data) {
    ...
    self.swagger = data;
  }).catch(catchFn).then(function (data) {
...
SwaggerClient.prototype.refreshOAuth = function (cb) {
  ...
  Request.post(definition.tokenUrl, { form: form, json: true }, function (err, resp, body) {
    if (err) return cb(err);
    if (resp.statusCode >= 300) return cb(new Error("Error refreshing oauth token:" + resp.statusCode));
    if (!body.access_token) return cb(new Error("No access token returned"));
...
SwaggerClient.prototype.request = function (method, path, answers, callback) {
  ...
  req.qs = url.query;
  var addParameter = function addParameter(param, ans) {
    if (param.in === 'header') {
      ...
      if (param.name === 'body') {
        req.body = ans;
        if ((typeof ans === 'undefined' ? 'undefined' : _typeof(ans)) === 'object') {
  ...
  params.forEach(function (param) {
    ...
    if (typeof ans === 'undefined' && param.in !== 'path') {
      return;
    }
  ...
  var addSecurity = function addSecurity(security) {
    ...
    if (def.type === 'apiKey' && self.auth[security.definition.name]) {
      ...
      if (def.flow === 'implicit') addParameter({ in: 'query', name: def.name || 'access_token' }, oauthToken);else if (def.name) addParameter({ in: 'query', name: def.name }, oauthToken);else addParameter({ in: 'header', name: 'Authorization' }, 'Bearer ' + oauthToken);
      oauthToken = '';
  ...
  if (self.middleware) {
    self.middleware.forEach(function (mw) {
      req = mw(req);
    });
  }
  ...
  Request(req, function (err, resp, body) {
    ...
    if (type.indexOf('json') !== -1) {
      ...
      implicitError = maybeGetResponseError(body);
    }
    if (resp.statusCode >= 300) {
      var errMsg = resp.statusCode + " - " + resp.statusMessage;
      if (body) {
        errMsg += '\n';
